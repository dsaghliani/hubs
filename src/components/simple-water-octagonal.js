...
export default class SimpleWaterOctagonal extends Mesh {
  constructor(normalMap, resolution = 24, lowQuality = false) {
    ...
    const radius = apothem / Math.cos(theta);

    // thetaLength is set to 365 degrees because, otherwise, a very narrow slice is left unfilled.
    const geometry = new CircleBufferGeometry(radius, sides, theta, THREE.Math.degToRad(365));
  ...
  clone(recursive) {
    return new SimpleWaterOctagonal(this.material.normalMap, this.resolution, this.lowQuality).copy(this, recursive);
  }
...
AFRAME.registerComponent("simple-water-octagonal", {
  schema: {
  ...
  init() {
    ...
    this.water = new SimpleWaterOctagonal(waterNormalMap, undefined, usePhongShader);
